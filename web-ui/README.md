...
这是积分激励项目的本地演示页面，包含奖品中心、兑换记录、可信身份验证和 Agent 对话。页面读取实时积分、奖品资格与订单状态，但不直接调用兑换写接口。
...
- `/v1/orders` 返回当前可信用户的兑换记录；存在处理中订单时页面每两秒刷新，全部进入终态后停止。
...
用户端入口为 `/`，智能运营工作台入口为 `/operator`。两者分别使用用户 JWT 和独立运营令牌，浏览器会将令牌保存在不同的 `sessionStorage` 键中，避免身份混用。运营工作台支持连续对话、实时快照、运营知识检索、活动草案、人工审核、确定性发布和效果回写；真实用户触达尚未接入。
