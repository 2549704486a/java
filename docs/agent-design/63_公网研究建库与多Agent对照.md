...
用户随后明确以整体 `B > C > A > D` 和文字理由代替细分评分。原话保存到评估目录的 `qualitative-review.json`，再核对映射生成 `qualitative-comparison.md`。这种调整不需要用户为了内部评分结构补充信息，也不能由 Agent 把排序换算成四维分数。

真实对应为 B=Codex 直接研究、C=Codex 显式委派、A=项目单 Agent、D=项目多 Agent。用户认为 B 最贴合项目，C 内容质量较高但客群不准确，A 相关性与价格信息不足且缺少客群，D 完全失败。

十二份运行复核显示：B、C 各产出十五条并通过确定性门禁；A 产出五条、门禁通过一条，活动无结构化候选且客群失败；D 三份均在规划预算校验处失败。C 通过门禁但仍被用户指出内容问题，说明门禁不等于语义正确性或业务适用性验收，具体错误条目仍待逐条核查。

原定量协议因缺少分数保持未完成，不能伪造量化报告；补充定性报告已经完成。D 相比 A 的可批准候选增量为 -1，已不满足至少 +2 的必要条件，因此本轮不能建议保留多 Agent。实际建议保留单 Agent 基础并优先参考 B 的资料，代码收敛与候选批准导出仍是后续步骤。跨系统排序只说明本轮实际效果，不能把差距全部归因于多 Agent 架构。
