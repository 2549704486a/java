...
默认只读根目录 `AGENTS.md`，其余文档按当前问题选读。不要顺序通读本目录或沿链接自动加载背景。
...
| `agent-design/20_Agent课程能力落地路线图.md` | 历史规划，默认不读取、不维护，仅用户明确要求时查看 |
...
| `agent-design/51_活动执行与效果观测技术方案.md` 至 `60_Java业务服务MCP最小样本.md` | 活动执行、实验、投放、效果分析、运行时 Harness、Prompt/Skill 纠偏，以及 Java MCP 最小样本 |

这些文档服务于开发、调试和代码评审，可以记录具体路径、接口、状态码和运行命令。

开始或恢复开发时，复用用户已确认的当前任务和已有上下文。路线图、课程索引及全部课程资料默认不加载、不维护，只有用户在本次提问中明确要求加载时才读取指定部分。

任务完成后仅更新直接受影响的规格或说明，不例行同步课程覆盖表、路线图或任务日记；日记仅在用户要求或阶段性交接需要时简记。
