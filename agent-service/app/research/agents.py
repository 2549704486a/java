...
class ResearchToolSession:
    ...
    def _build_tools(self) -> list[BaseTool]:
        ...
        def search_public_web(query: str) -> dict[str, Any]:
            ...
            if self._search_call_count >= self._brief.budget.max_search_queries:
                return {
                    "ok": False,
        ...
        def read_public_page(url: str) -> dict[str, Any]:
            ...
            self._tool_call_count += 1
            if self._read_call_count >= self._brief.budget.max_pages_to_read:
                return {
                    "ok": False,
    ...
    def _next_source_id(self) -> str:
        ...
        return f"source-page-{self._source_sequence:03d}"
    ...
    def run(
        ...
        *,
        ...
        if self._finalizer is None:
            ...
            finalizer_model_calls, _, _ = _read_model_usage(finalization_result)
    ...
    collector = create_agent(
        ...
        middleware=[
            ToolCallLimitMiddleware(
                ...
                run_limit=session.brief.budget.max_search_queries,
                exit_behavior="end",
            ),
            ToolCallLimitMiddleware(
                tool_name="read_public_page",
                run_limit=session.brief.budget.max_pages_to_read,
                exit_behavior="end",
            ),
        ],
...
def _read_model_usage(result: dict[str, Any]) -> tuple[int, int | None, int | None]:
    model_messages = [
        message
...
def _read_all_model_usage(
    ...
    usages = [_read_model_usage(result) for result in results]
