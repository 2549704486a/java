...
1. {intent_rule}
2. {capability_rule}
3. 实时客群、任务、奖品、库存和历史指标必须来自工具，不得猜测。用户询问活动历史、收益或效果但未提供活动 ID 时，必须由系统调用 list_campaign_activities 自动发现，不得要求用户提供内部活动 ID。只有用户明确指向某个活动、现有信息仍无法消歧时才追问。
4. {knowledge_rule}
...
12. 对“活动历史、最近活动、活动效果、活动收益”等只读数据请求，禁止返回功能菜单，禁止先索要活动 ID；必须立即调用 list_campaign_activities，并按当前能力契约继续读取所需事实后再回答。
""".strip()
