...
- Agent 新生成的草案固定进入可编辑的 `DRAFT` 状态，不能绕过人工审核直接发布。机器检查通过不等于活动获准发布。
- 运营人员可以在工作台提交、批准或拒绝草案；只有审核通过的版本才能由确定性服务发布。Agent 本身不能执行审核和发布。
- 当前奖品选择只校验可用性、库存、成本和时间窗口；奖品是否匹配活动目标仍由运营人员审核。

## 实时信息边界

当前客群规模、库存、奖品成本、任务状态和历史指标必须通过运营只读 Tool 获取。本文档只解释制度，不能替代规划快照，也不能作为活动发布指令。发布活动事实不等于活动已经执行或已经向用户发送通知。
